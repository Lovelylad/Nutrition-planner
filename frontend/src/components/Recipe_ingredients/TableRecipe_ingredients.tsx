...
import { loadColumns } from './configureRecipe_ingredientsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListRecipe_ingredients from './ListRecipe_ingredients';

const perPage = 10;

const TableSampleRecipe_ingredients = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: recipe_ingredientsNotify,
    refetch,
  } = useAppSelector((state) => state.recipe_ingredients);
  ...
  useEffect(() => {
    if (recipe_ingredientsNotify.showNotification) {
      notify(
        recipe_ingredientsNotify.typeNotification,
        recipe_ingredientsNotify.textNotification,
      );
    }
  }, [recipe_ingredientsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/recipe_ingredients/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/recipe_ingredients/recipe_ingredients-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {recipe_ingredients && Array.isArray(recipe_ingredients) && !showGrid && (
        <ListRecipe_ingredients
...
export default TableSampleRecipe_ingredients;
