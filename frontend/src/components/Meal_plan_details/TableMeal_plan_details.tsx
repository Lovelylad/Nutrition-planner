...
import { loadColumns } from './configureMeal_plan_detailsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListMeal_plan_details from './ListMeal_plan_details';

const perPage = 10;

const TableSampleMeal_plan_details = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: meal_plan_detailsNotify,
    refetch,
  } = useAppSelector((state) => state.meal_plan_details);
  ...
  useEffect(() => {
    if (meal_plan_detailsNotify.showNotification) {
      notify(
        meal_plan_detailsNotify.typeNotification,
        meal_plan_detailsNotify.textNotification,
      );
    }
  }, [meal_plan_detailsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/meal_plan_details/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/meal_plan_details/meal_plan_details-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {meal_plan_details && Array.isArray(meal_plan_details) && !showGrid && (
        <ListMeal_plan_details
...
export default TableSampleMeal_plan_details;
