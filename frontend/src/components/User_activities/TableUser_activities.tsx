...
import { loadColumns } from './configureUser_activitiesCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListUser_activities from './ListUser_activities';

const perPage = 10;

const TableSampleUser_activities = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: user_activitiesNotify,
    refetch,
  } = useAppSelector((state) => state.user_activities);
  ...
  useEffect(() => {
    if (user_activitiesNotify.showNotification) {
      notify(
        user_activitiesNotify.typeNotification,
        user_activitiesNotify.textNotification,
      );
    }
  }, [user_activitiesNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/user_activities/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/user_activities/user_activities-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {user_activities && Array.isArray(user_activities) && !showGrid && (
        <ListUser_activities
...
export default TableSampleUser_activities;
