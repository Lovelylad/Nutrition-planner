  ...
  static async search(searchQuery, currentUser, organizationId, globalAccess) {
    try {
      ...
      const tableColumns = {
        ...
        foods: ['name', 'macronutrients', 'micronutrients', 'serving_size'],

        meal_plan_details: ['day'],

        meal_plans: ['name'],

        organizations: ['name', 'address', 'phone'],

        profiles: ['dietary_restrictions', 'health_goals'],

        recipes: ['title', 'instructions', 'serving_info', 'image_url'],
      ...
      const columnsInt = {
        ...
        recipes: ['preparation_time', 'cooking_time'],
      };
